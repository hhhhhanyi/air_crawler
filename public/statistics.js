...
.then((calendarAPI)=>{
  if(calendarAPI.status === "error"){
    ...
    for (let k=0; k<calendarAPI.calendar.length; k++){
      calendar_date = (parseInt(k+1, 10) + 100).toString().substr(1)
      calendar += `<li><a href="/result.html?departure=${departureCode}&arrival=${arrivalCode}&date=${year}-${month}-${calendar_date}&p=${person}&t=${type}">
        <p class="calendar_date">${calendar_date}</p>
        `
      ...
      google.charts.setOnLoadCallback(drawMarkersMap);
       ...
       google.visualization.events.addListener(chart, 'select', function () {
          ...
          if (selection.length > 0) {
            ...
            let selected_arrival;
            for(let i=0; i<place.length; i++){
              if (data.getValue(selection[0].row, 0) === place[i].split("_")[1]){
                selected_arrival = place[i].split("_")[0];
              }
            }
            window.location.href=`/result.html?departure=${departureCode}&arrival=${selected_arrival}&date=${date}&p=${person}&t=${type}`;
          }
        });
